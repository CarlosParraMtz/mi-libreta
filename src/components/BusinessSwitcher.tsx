  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(firestore, 'users', user.uid), (snapshot) => {
      ...
      void Promise.all(ids.map(async (id) => {
        ...
        return business.exists() ? { id, name: String(business.data().businessName || 'Libreta sin nombre') } : null
      })).then((items) => {
