...
export function Onboarding() {
  ...
  const [businessType, setBusinessType] = useState(current.businessType)
  const [phone, setPhone] = useState(current.phone)
  const [modules, setModules] = useState<ProductModule[]>(current.enabledModules.length ? current.enabledModules : ['credits'])
  ...
  const next = () => { if (step === 0 && (!businessName.trim() || !ownerName.trim() || !businessType)) { setError('Completa el nombre, el encargado y el tipo de negocio.'); return } if (step === 1 && !modules.length) { setError('Elige al menos una herramienta para tu libreta.'); return } setError(''); setStep((value) => Math.min(2, value + 1)) }
  const complete = async () => {
    setSaving(true); setError('')
    try {
      const created = await createBusinessForCurrentUser({ businessName: businessName.trim(), ownerName: ownerName.trim(), businessType, phone, enabledModules: modules })
      setLedger(created)
      navigate('/dashboard')
    } catch (issue) {
  ...
  return <main className="min-h-screen bg-cream px-4 py-6 text-ink sm:px-8 sm:py-10"><div className="mx-auto max-w-3xl"><header className="flex items-center justify-between"><span className="text-2xl font-black"><span className="text-coral">Mi</span> Libreta</span><p className="text-sm font-extrabold text-ink/35">Paso {step + 1} de 3</p></header><div className="mt-6 flex gap-2">{[0, 1, 2].map((item) => <span key={item} className={`h-2 flex-1 rounded-full transition ${item <= step ? 'bg-coral' : 'bg-ink/10'}`} />)}</div><AnimatePresence mode="wait"><motion.section key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: .2 }} className="mt-7 rounded-[2rem] bg-white p-6 shadow-card sm:p-9">
    {step === 0 && <><span className="grid size-13 place-items-center rounded-2xl bg-peach"><Store size={24} /></span><h1 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">Cuéntanos de tu negocio</h1><p className="mt-2 font-semibold text-ink/45">Usaremos esto para que la libreta se sienta realmente tuya.</p><div className="mt-7 grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">Nombre del negocio</span><input value={businessName} onChange={(event) => setBusinessName(event.target.value)} className={authInputClass} placeholder="Ej. Abarrotes Lupita" autoFocus /></label><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">¿Quién lo atiende?</span><input value={ownerName} onChange={(event) => setOwnerName(event.target.value)} className={authInputClass} placeholder="Ej. Luisa" /></label><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">Tipo de negocio</span><select value={businessType} onChange={(event) => setBusinessType(event.target.value)} className={authInputClass}><option value="">Selecciona uno</option>{businessTypes.map((type) => <option key={type}>{type}</option>)}</select></label><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">WhatsApp del negocio</span><input value={phone} onChange={(event) => setPhone(event.target.value)} className={authInputClass} inputMode="tel" placeholder="10 dígitos" /></label></div></>}
