import { Check, CreditCard, LogOut, MailPlus, Save, Store, Users } from 'lucide-react'
...
export function Settings() {
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    update({ businessName: String(form.get('businessName')), ownerName: String(form.get('ownerName')), businessType: String(form.get('businessType')), phone: String(form.get('phone')), enabledModules: modules })
  ...
  return <main className="px-4 pb-8 pt-6 sm:px-8 sm:pt-9 lg:px-12"><div className="mx-auto max-w-4xl"><PageHeader eyebrow="Tu cuenta" title="Configuración" />
    ...
    <form onSubmit={save} className="mt-6 space-y-5">
      <section className="rounded-[2rem] bg-white p-5 shadow-soft sm:p-7"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-peach"><Store size={21} /></span><div><h2 className="text-xl font-black">Datos del negocio</h2><p className="text-sm font-semibold text-ink/40">Así aparecerá dentro del dashboard.</p></div></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">Negocio</span><input name="businessName" defaultValue={ledger.businessName} className={authInputClass} required /></label><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">Encargado</span><input name="ownerName" defaultValue={ledger.ownerName} className={authInputClass} required /></label><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">Tipo de negocio</span><input name="businessType" defaultValue={ledger.businessType} className={authInputClass} required /></label><label><span className="mb-1.5 block text-sm font-extrabold text-ink/60">WhatsApp</span><input name="phone" defaultValue={ledger.phone} className={authInputClass} inputMode="tel" /></label></div></section>
    ...
    <section className="mt-5 rounded-[2rem] bg-white p-5 shadow-soft sm:p-7"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-sky"><Users size={21} /></span><div><h2 className="text-xl font-black">Administradores de la libreta</h2><p className="text-sm font-semibold text-ink/40">Invita a alguien para que pueda atender y configurar el mismo negocio.</p></div></div><form onSubmit={invite} className="mt-6 flex flex-col gap-3 sm:flex-row"><input name="email" type="email" className={authInputClass} placeholder="correo@ejemplo.com" required /><button disabled={inviteLoading || !businessId} className="flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-sky px-5 py-3.5 font-black disabled:opacity-50"><MailPlus size={18} /> {inviteLoading ? 'Invitando…' : 'Invitar'}</button></form>{inviteUrl && <div className="mt-4 rounded-2xl bg-cream p-4"><p className="text-xs font-black text-ink/45">SES aún no está configurado. Comparte este enlace:</p><a href={inviteUrl} className="mt-1 block break-all text-sm font-bold text-coral">{inviteUrl}</a></div>}<p className="mt-3 text-xs font-semibold text-ink/35">La persona debe entrar con ese mismo correo. Si ya tiene cuenta, el acceso se agrega inmediatamente.</p></section>

    <section className="mt-5 rounded-[2rem] bg-ink p-5 text-white shadow-card sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-lime text-ink"><CreditCard size={21} /></span><div><p className="text-xs font-bold text-white/45">Suscripción</p><h2 className="text-xl font-black">{subscriptionLabels[subscription.status]}</h2>{subscription.currentPeriodEnd && <p className="text-xs font-semibold text-white/40">Periodo hasta {new Intl.DateTimeFormat('es-MX').format(new Date(subscription.currentPeriodEnd))}</p>}</div></div><button disabled={billingLoading || !businessId} onClick={() => billing(hasStripeCustomer ? 'portal' : 'checkout')} className="rounded-2xl bg-lime px-5 py-3.5 font-black text-ink disabled:opacity-50">{billingLoading ? 'Abriendo…' : hasStripeCustomer ? 'Administrar en Stripe' : 'Activar suscripción'}</button></div>{subscription.accessOverride === 'suspended' && <p className="mt-4 rounded-xl bg-coral/20 p-3 text-sm font-bold text-peach">El acceso está suspendido por un administrador de la plataforma.</p>}</section>
