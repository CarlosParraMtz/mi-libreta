...
export function Register() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { const result = await createUserWithEmailAndPassword(auth, email, password); await updateProfile(result.user, { displayName: name }); navigate(await signedInDestination(result.user, '/onboarding')) } catch (issue) { setError(authErrorMessage(issue)); setLoading(false) }
  }
  const google = async () => {
    setLoading(true); setError('')
    if (!auth) { setError('Configura Firebase para crear una cuenta.'); setLoading(false); return }
    try { const result = await signInWithPopup(auth, new GoogleAuthProvider()); const isNewUser = getAdditionalUserInfo(result)?.isNewUser === true; if (isNewUser) prepare(result.user.displayName || 'Dueño'); navigate(await signedInDestination(result.user, isNewUser ? '/onboarding' : returnTo)) } catch (issue) { setError(authErrorMessage(issue)); setLoading(false) }
  }
